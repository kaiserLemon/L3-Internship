...
N = 202   #number of vertices in the graph. Only used in the reward function, not directly relevant to the algorithm 
MYN = N-2  #The length of the word we are generating. Here we are generating a graph, so we create a 0-1 word of length (N choose 2)
...
n_actions = N #The size of the alphabet. In this file we will assume this is 2. There are a few things we need to change when the alphabet size is larger,
...
def calcScore(state):
	...
	G = nx.from_prufer_sequence(state[:MYN])
	...
	distMat = nx.algorithms.shortest_paths.dense.floyd_warshall_numpy(G)
	...
	if ans > 0:
		...
		print("valeur de proximity: " + str(proximity) +  "\n valeur de l'autre membre: " + str(evals[math.floor(2*diam/3)-1]))
