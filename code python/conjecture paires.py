...
N = 42 #number of vertices in the graph. Only used in the reward function, not directly relevant to the algorithm 
...
n_actions = 3 #The size of the alphabet. In this file we will assume this is 2. There are a few things we need to change when the alphabet size is larger,
...
def calcScore(state):
	...
	has_cycles = has_Cycles(cycles)
	if has_cycles == True:
		return -INF

	
	def is_dominating_pair(G, u, v):
		"""Returns True if (u, v) is a dominating pair in G."""
		# Check if u and v have any common neighbors
		result = False 
		for w in G.nodes():
			if u != v and v != w : 
	...
	def count_dominating_pairs(G):
		"""Returns the number of dominating pairs in G."""
		num_dominating_pairs = 0
		num_pairs = G.number_of_nodes() * (G.number_of_nodes() - 1)
		for u in G.nodes():
			for v in G.nodes():
				if u != v and is_dominating_pair(G, u, v):
					num_dominating_pairs += 1
		if num_dominating_pairs > num_pairs:
			raise ValueError("Number of dominating pairs exceeds total number of pairs")
		return num_dominating_pairs

	dom_pairs = count_dominating_pairs(G)
	max_pairs = N*(N-1)

	myScore = (dom_pairs / max_pairs)
	if myScore > 0.94:
...
for i in range(1000000): #1000000 generations should be plenty
	...
	print(	"Mean reward: " + str(mean_all_reward) + "\nSessgen: " + str(sessgen_time) + ", other: " + str(randomcomp_time) + ", select1: " + str(select1_time) + ", select2: " + str(select2_time) + ", select3: " + str(select3_time) +  ", fit: " + str(fit_time) + ", score: " + str(score_time) + ", number of iterations: " + str(i)) 
