...
LEARNING_RATE = 0.001
n_sessions =500 #number of new individuals per generation
...
def calcConj():
    ...
    distMatGen = dict(nx.all_pairs_shortest_path_length(path))
    distMat = np.zeros([N,N])
    for i in range(N):
        for j in range(N):
            distMat[i][j]=distMatGen[i][j]
    charPoly = np.poly(distMat)
    for i in range(3,len(charPoly)):
        charPoly[i] = charPoly[i] / (2**(i-2))
    peak = np.argmin(charPoly)
    return peak
...
def calcScore(state):
    ...
    charPoly = np.poly(distMat)
    charPolyA = np.poly(nx.adjacency_matrix(tree).todense())
    charPolyB = []
    for i in range(len(charPolyA)):
        if (abs(charPolyA[i]) > 0.1) and (i%2==0):
            charPolyB.append(abs(charPolyA[i]))

    
    if len(charPolyB) < 4:
        return 0
    
    if not unimodal(charPolyB):
        print(tree.edges())
        print(charPolyA)
        print(charPolyB)
        exit()
        return 100000
    peakAdj = np.argmax(charPolyB)
    real_peakAdj = peakAdj
    #print(charPolyB,peakAdj,len(charPolyB))
    #print(peakAdj, len(charPolyB),charPolyB,charPolyA)
    peakAdj = peakAdj / len(charPolyB)
    #print(peakAdj,len(charPolyB))
    ...
    if not unimodal(charPoly):
        return 100000
        
    peak = np.argmin(charPoly)
    #print(charPoly,peak,len(charPoly))
    #print(peak,len(charPoly))
    peak = (peak + 0.0)/N
    myResult = abs(peak-peakAdj)
    ...
    if (myResult > 0.299) :
        ...
        print(charPolyB)
        print(charPoly)
        print("real peakAdj: " + str(real_peakAdj) + "real peak: " + str(peak))
        print(tree.edges())
        print(nx.adjacency_matrix(tree).todense())
        nx.draw_kamada_kawai(tree)
        plt.show()
        print("\n")
        exit()
    
    return abs(peakAdj-peak )
